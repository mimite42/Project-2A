...
def move_to_position(move_group, x, y, z):
    ...
    rospy.sleep(4.5)  # Use rospy.sleep() for a more ROS-friendly delay

    # Set only the wrist_2_joint while keeping others unchanged
    joint_values = initial_joint_values
    joint_values[4] = -1.5708  # Set wrist_2_joint to a constant value of -1.5708
    ...
    while not rospy.is_shutdown():
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                # Move the robot to the marker position with wrist_2_joint fixed
